...
import { hrCheckEmployeePresenceByEmployeeId } from "../methods/hrCheckEmployeePresenceByEmployeeId.js";
import { hrCheckEmployeePresenceByEmployeeIdSchema } from "../schemas/hrCheckEmployeePresenceByEmployeeIdSchema.js";

export default fp(async function adminCheckEmployeePresenceByEmployeeIdRoute(fastify) {
  fastify.post("/hr/check-employee-presence", async (request, reply) => {
    try {
      const parsed = hrCheckEmployeePresenceByEmployeeIdSchema.safeParse(request.body);
      ...
      const result = await hrCheckEmployeePresenceByEmployeeId(authHeader, parsed.data);
