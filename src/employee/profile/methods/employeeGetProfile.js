...
export async function employeeGetProfile(authHeader) {
	...
	const assignedShift = details.assignedShiftId
		? await (async () => {
				const shift = await prisma.shift.findUnique({
					where: {
						id: details.assignedShiftId,
					},
				});
				...
				const fullStart = dayjs
					.utc(shift.fullShiftStartingTime)
					.tz(TIMEZONE);

				const fullEnd = dayjs
					.utc(shift.fullShiftEndingTime)
					.tz(TIMEZONE);
				...
				const halfDayMinFullTime = fullStart.add(
					halfDayMinFull,
					"minute"
				);

				const halfDayMaxFullTime = fullStart.add(
					halfDayMaxFull,
					"minute"
				);
