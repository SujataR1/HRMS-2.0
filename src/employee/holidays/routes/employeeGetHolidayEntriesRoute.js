...
import { employeeGetHolidayEntries } from "../methods/employeeGetHolidayEntries.js";
import { employeeGetHolidayEntriesSchema } from "../schemas/employeeGetHolidayEntriesSchema.js";

export default fp(async function hrGetHolidayEntriesRoute(fastify) {
	fastify.post("/employee/holidays/view", async (request, reply) => {
		...
		const parsed = employeeGetHolidayEntriesSchema.safeParse(request.body);
		...
		try {
			const holidays = await employeeGetHolidayEntries({
				authHeader,
				...parsed.data,
			});
